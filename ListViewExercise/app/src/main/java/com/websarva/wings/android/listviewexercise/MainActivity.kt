package com.websarva.wings.android.listviewexercise
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (i in 0 until 100) {
            ...
            if(i % 5 == 2){
                var menu = mutableMapOf("name" to "ミックスピザ", "price" to "1000円")
                menuList.add(menu)
            }
            if(i % 5 == 3){
                var menu = mutableMapOf("name" to "海鮮丼", "price" to "1100円")
                menuList.add(menu)
            }
            if(i % 5 == 4){
                var menu = mutableMapOf("name" to "焼肉弁当", "price" to "1200円")
                menuList.add(menu)
            }
        }
